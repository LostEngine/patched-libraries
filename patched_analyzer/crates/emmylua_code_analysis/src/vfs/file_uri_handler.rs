...
// LostEngine -- modify file_path_to_uri
pub fn file_path_to_uri(path: &PathBuf) -> Option<Uri> {
    let path_str = path.to_string_lossy().replace("\\", "/");

    let scheme_path = if path_str.starts_with('/') {
        format!("file://{}", path_str)
    } else {
        format!("file:///{}", path_str)
    };

    Uri::from_str(&scheme_path).ok()
}

// LostEngine -- modify uri_to_file_path
pub fn uri_to_file_path(uri: &Uri) -> Option<PathBuf> {
    ...
    let decoded_path = if cfg!(target_arch = "wasm32") {
